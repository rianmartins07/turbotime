...
export default function NoteEditor() {
    ...
    async function load() {
      try {
        ...
        if (categories.length === 0) {
          categoriesData = await notesAPI.getCategories()
          if (isMounted) setCategories(categoriesData)
        }

        if (id === 'new') {
          const defaultCategory = categoriesData[0]?.name || 'Random Thoughts'
        ...
        if (isMounted && id !== 'new') setIsLoading(false)
      }
    }
  ...
  const autoSave = async (updatedFields) => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        setIsSaving(true)
        const saved = await notesAPI.patch(currentId, updated)
  ...
  const categoryColor = categoryData?.color || categories[0]?.color || '#ef9c66'
